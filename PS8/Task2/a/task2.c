...
void allocator_init(struct allocator *ptr, int size){
	...
	ptr->count = 0;
	ptr->size = size;
	ptr->max_memory =  (char*) malloc(sizeof(char) * (number_of_allocations * number_of_threads));
	ptr->next = NULL;
...
void *memory_allocation(void *arg){
	...
	struct allocator *alloc_ptr = (struct allocator*)arg;
	pthread_mutex_lock(&lock);
	printf("Add to Allocator\n");
	for(int i = 0; i < number_of_allocations; i++){
		allocator_add(alloc_ptr, size_in_byte);
	}
...
int main(int argc, char *argv[]){
	...
	allocator_init(ptr, (number_of_threads * number_of_allocations * size_in_byte));
	...
	for(int i = 0; i < number_of_threads; i++){
		...
		check = pthread_create(&thread_id[i], NULL, &memory_allocation, ptr);
		if(check == -1){
	...
	//Test: Delete one and try to add two
	allocator_delete(ptr, size_in_byte);
	allocator_add(ptr, size_in_byte);
	allocator_add(ptr, size_in_byte),


	/*//free memory
	for(int i = 0; i < (number_of_threads * number_of_allocations); i++){
		free(ptr->max_memory[i]);
	}
*/
